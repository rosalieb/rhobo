...
# Note that if we had previous correction factor in the folder, they would also be shown here with pCF.all().
pCF.all(metadata_QAQC)
```
...
The function `rhobo.treatment()` can then be used to assign the treatments (read `?rhobo.treatment` for all the options) and do some more detailed plots.
